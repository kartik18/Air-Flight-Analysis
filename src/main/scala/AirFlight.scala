...
object AirFlight {
  def main(args: Array[String]): Unit = {
   ...
   // val arrivalFlightDelayeddf = spark.sparkContext.parallelize(Seq(arrivalFlightDelayed))
    ...
    val join1FlightByWeekdf = DelayedFlightByWeek.join(OnTimeFlightByWeek,Seq("DAY_OF_WEEK"))
    val join2FlightByWeekdf = join1FlightByWeekdf.join(ArrivalFlightByWeekdf,Seq("DAY_OF_WEEK"))
    val join3FlightByWeekdf = join2FlightByWeekdf.join(AheadOfTimeFlightByWeek,Seq("DAY_OF_WEEK"))
   ...
   outcomedf.write.format("orc").mode(SaveMode.Append).saveAsTable("default.Analytics")
    println("Done")
  }
}
